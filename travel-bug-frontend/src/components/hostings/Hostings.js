...
const Hostings = () => {
  ...
  {
    if (flag) {setHostingDuration({ days: hostingDuration.days + 1, hours: hostingDuration.hours})}
    else {
  ...
  {
    if (flag) {setHostingDuration({ days: hostingDuration.days, hours: Math.min(hostingDuration.hours + 1, 18)})}
    else {
  ...
  {
    if (flag) {setDayTimeSlot({end: dayTimeSlot.end, start: Math.min(dayTimeSlot.start + 1, 23)})}
    else {
      setDayTimeSlot({start: Math.max(dayTimeSlot.start - 1,0), end: dayTimeSlot.end})
    }
  }

   const setDayTimeSlotEnd = (flag)=>
  {
    if (flag) {setDayTimeSlot({start: dayTimeSlot.start, end: Math.min(dayTimeSlot.end + 1, 23)})}
    else {
      setDayTimeSlot({end: Math.max(dayTimeSlot.end - 1,0), start: dayTimeSlot.start})
    }
  }
